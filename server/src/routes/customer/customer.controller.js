const { createCustomerAccount, logInCustomer, addCustomerAddress, addCustomerScan, getAllCustomerAddressesById, deleteCustomerAddressById } = require("../../models/customer.model");
...
async function httpAddCustomerScan (req, res) {
    await addCustomerScan(req.body, function(err, data) {
        if (err) {
            res.status(400).json(err)
        } else {
            res.status(200).json(data)
